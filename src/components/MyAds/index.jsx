...
import BtnPraimary from '../BtnPraimary'
            ...
            <div className={styles.info}>
                <div className={styles.container}>
                    ...
                    {btnTitle.map((item,index)=>(
                        <div className={index===btnActive ? styles.container__button__active : styles.container__button} key={index} onClick={()=>{setBtnActive(index)}}>{item}</div>
                    ))}
                </div>
                <div className={styles.subscribe}>
                    <h3>Ваша подписка</h3>
                    <p className={styles.subscribe__title}>{title}</p>
