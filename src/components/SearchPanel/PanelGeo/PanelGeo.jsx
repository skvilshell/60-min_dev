...
export default function PanelGeo({ setActiveEl, activeEl }) {
   ...
   const currentTitle = useSelector(state => state.filter.geo.title)

   return (
      <div ref={refEl} className={s.root}>
         <div className={s.tab}>
            <div
               ...
               <div className={s.dropdown_content}>
                  {
                     (variant ? elements2 : elements).map((item, index) => (
                        <div
                           key={index}
                           className={s.dropdow_element + " " + ((item.title === (variant ? geoL : geoM) || (item.title === currentTitle)) && s.active)}
                           onClick={() => {
